...
class SalleCinemaController extends AbstractController
{
    ...
    public function new(
        ...
        $event = $entityManager
            ...
            ->find($request->query->get('evenement'));
        $salleCinema
            ->setIdEvent($event)
            ->setChat('')
            ->setUrlSalle('');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $result = json_decode(
                $this->getRoom($salleCinema->getUrlFilm(), 'share'),
                true
            );
    ...
    {
        $user = $request->getSession()->get('userdata');
        $date = explode('/', $salleCinema->getIdEvent()->getDateEvent());
    ...
    private function getRoom($share, $type)
    {
        ...
        $key =
            '3fqimojunmdvza30y3ve1numvukr7ivhe2t5pc7j76h29o7xr1aoqi432fus7fgm';
        $data = [
            'w2g_api_key' => $key,
            'share' => $share,
            'bg_color' => '#008080',
